# Applications Board — Business Specification

### Overview
The Applications Board provides a visual Kanban-style board for recruiters and administrators to manage candidates efficiently. It transforms the list-based tracking into a dynamic interface where candidates are represented as cards across horizontal lanes corresponding to their hiring stage. 
...
- As a **company administrator**, I want to drag a candidate's card to an 'Archive' zone or click an archive action on the card, so that I can remove them from the active applications board when a final decision is reached.
...
- As a **company administrator**, I want to filter the board by job posting, so that I can focus on the applications board for a specific role.
