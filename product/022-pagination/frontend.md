# 022: Pagination — Frontend Design

## Angular Models

We will create a generic interface `PagedResponse<T>` to match the backend contract:
...
We will create a reusable standalone component `PaginationComponent` (`src/app/core/ui/pagination/pagination.component.ts`) that matches the styling defined in `product/design-system.md`:
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex items-center gap-2">
            ...
            <select 
              ...
              class="rounded-md border border-slate-300 py-1.5 px-2 text-sm font-medium text-slate-700 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
...
For listing components (e.g., Requisitions list, Candidate Applications list):
- Introduce Signals for state management:
```typescript
page = signal(1);
pageSize = signal(20);
totalCount = signal(0);
totalPages = signal(1);
```
- When pagination change inputs trigger, update Signals and trigger service reload:
```typescript
...
```
- Integrate search & filters updates: when filters change, reset `this.page.set(1)` to ensure results start from page 1.
